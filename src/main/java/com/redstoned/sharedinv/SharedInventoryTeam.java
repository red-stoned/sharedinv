...
public class SharedInventoryTeam {
	public static HashMap<UUID, SharedInventoryTeam> playerTeams = new HashMap<>();
	...
	public SharedInventoryTeam(String name) {
		this.name = name;
	}

	private SharedInventoryTeam() {}

	public static SharedInventoryTeam fromNbt(RegistryWrapper.WrapperLookup rm, NbtCompound nbt) {
		SharedInventoryTeam t = new SharedInventoryTeam();

		t.name = nbt.getString("name");
		NbtList nbtPlayers = nbt.getList("players", 10);
		for (int i = 0; i < nbtPlayers.size(); ++i) {
			NbtElement ia = nbtPlayers.get(i);
			UUID p = NbtHelper.toUuid(ia);
			t.players.add(p);
			SharedInventoryTeam.playerTeams.put(p, t);
		}
	...
	public void AddPlayer(UUID uuid) {
		if (this.players.contains(uuid)) {
			SharedInventory.LOGGER.info("player to join team that they are already on: " + uuid);
			return;
		}

		SharedInventoryTeam existingTeam = playerTeams.get(uuid);
		if (existingTeam != null) {
			existingTeam.RemovePlayer(uuid);
		}

		this.players.add(uuid);
		playerTeams.put(uuid, this);
	}

	public void RemovePlayer(UUID uuid) {
		Boolean removed = this.players.remove(uuid);
		SharedInventory.LOGGER.info("removed uuid: " + removed);
	}

	public String Name() {
		return "sharedinv:" + this.name;
	}
}
